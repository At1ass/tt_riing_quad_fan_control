...
namespace gui {
    GTKTrayManager::GTKTrayManager() {
        ...
        gtk_thread = std::jthread([this]() {
            ...
            app_indicator_set_attention_icon_full(indicator.get(), "dialog-information", "New messages");
            ...
            g_signal_connect(toggle_item, "activate", G_CALLBACK(+[](GtkWidget*  /*widget*/, gpointer data) {
            ...
            g_signal_connect(quit_item, "activate", G_CALLBACK(+[](GtkWidget*  /*widget*/, gpointer data) {
                auto* self = static_cast<GTKTrayManager*>(data);
            ...
            app_indicator_set_menu(indicator.get(), GTK_MENU(menu));
    ...
    void GTKTrayManager::cleanup() {
        gtk_main_quit();
    }

    void GTKTrayManager::openFileDialog(FileDialogCallback callback) {
        openFileChooserDialog("Open File", GTK_FILE_CHOOSER_ACTION_OPEN, std::move(callback));
    }

    void GTKTrayManager::saveFileDialog(FileDialogCallback callback) {
        openFileChooserDialog("Save File", GTK_FILE_CHOOSER_ACTION_SAVE, std::move(callback));
    }

    void GTKTrayManager::openFileChooserDialog(const char* title, GtkFileChooserAction action, FileDialogCallback callback) {
        g_idle_add([](gpointer data) -> gboolean {
            auto* dialogData = static_cast<std::tuple<const char*, GtkFileChooserAction, FileDialogCallback>*>(data);

            // Создаём диалог файлового выбора
            GtkWidget* dialog = gtk_file_chooser_dialog_new(
                    std::get<0>(*dialogData),
                    nullptr,
                    std::get<1>(*dialogData),
                    "_Cancel", GTK_RESPONSE_CANCEL,
                    (std::get<1>(*dialogData) == GTK_FILE_CHOOSER_ACTION_SAVE ? "_Save" : "_Open"), GTK_RESPONSE_ACCEPT,
                    nullptr
                    );
        ...
        }, new std::tuple<const char*, GtkFileChooserAction, FileDialogCallback>(title, action, std::move(callback)));

        core::Logger::log_(core::LogLevel::INFO) << title << " dialog requested." << std::endl;
    }
}
